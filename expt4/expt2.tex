...
\begin{itemize}
\item We notice that there is a tradeoff between gain and linearity, as the gainincreases, the linearity decreases which is reflected from the THD.

\item As we fit the data with a better fitting function (DC transfer function) we see that the point where the maximum AC gain is obtained changes and the THD changes. We see that the Gaussian function is a better approximation that Fourier fit, and hence gives a better approximation of the DC bias point. As we move away from this point we notice that the THD increases.
\end{itemize}
